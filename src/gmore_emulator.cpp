...
void Emulator::forgetKittyTransmissions() const { impl_->forgetKittyTransmissions(); }
void Emulator::feed(const char* p, size_t n) { impl_->feed(p, n); }
size_t Emulator::contentRows() const { return impl_->contentRows(); }
std::vector<std::pair<int, int>> Emulator::matchSpans(size_t absRow, const std::regex& re) const {
    return impl_->matchSpans(absRow, re);
}
void Emulator::renderRow(size_t absRow, std::string& out, bool withImages,
                         size_t viewTop, size_t viewBot, bool withLinks,
...
const std::vector<Image>& Emulator::images() const { return impl_->images; }
