...
LangConfig makeCpp() {
    return {
        {"alignas","alignof","and","and_eq","asm","auto","bitand","bitor",
         ...
         "override","final","[[nodiscard]]","[[maybe_unused]]","[[likely]]","[[unlikely]]"},
        "//", "/*", "*/",
        /*tripleString=*/false, /*charLiterals=*/true, /*hasPreprocessor=*/true,
        /*digitSep=*/'\'',
    };
}
...
LangConfig makeC() {
    ...
    static const std::vector<std::string> cppOnly = {
        "alignof","and","and_eq","bitand","bitor","bool","catch","char8_t",
        "char16_t","char32_t","class","compl","concept","const_cast","co_await",
        "co_return","co_yield","decltype","delete","dynamic_cast","explicit",
        "export","friend","inline","mutable","namespace","new","noexcept",
    ...
    kw.erase(std::remove_if(kw.begin(), kw.end(), [&](const std::string& k){
        return std::find(cppOnly.begin(), cppOnly.end(), k) != cppOnly.end();
    }), kw.end());
    // Add C23 keywords
    for (auto& k : std::vector<std::string>{"_Alignas","_Alignof","_Atomic","_Bool",
            "_Complex","_Generic","_Imaginary","_Noreturn","_Static_assert",
            "_Thread_local","nullptr","true","false","constexpr"})
        kw.push_back(k);
    cfg.digitSep = '\'';
    return cfg;
}
...
LangConfig makePython() {
    return {
        {"False","None","True","and","as","assert","async","await","break",
         ...
         "isinstance","issubclass","super","object","int","float","str",
         "list","dict","set","tuple","bool","bytes","bytearray","memoryview",
         "open","input","abs","round","min","max","sum","sorted","reversed",
         "any","all","vars","dir","getattr","setattr","hasattr","delattr",
         "staticmethod","classmethod","property"},
        "#", "", "",
        /*tripleString=*/true, /*charLiterals=*/false, /*hasPreprocessor=*/false,
        /*digitSep=*/'_',
    };
}
...
LangConfig makeJavaScript() {
    return {
        {"abstract","arguments","async","await","boolean","break","byte","case",
         ...
         "finally","float","for","from","function","get","goto","if","implements",
         ...
         "while","with","yield"},
        "//", "/*", "*/",
        /*tripleString=*/false, /*charLiterals=*/false, /*hasPreprocessor=*/false,
        /*digitSep=*/'_',
    };
}
...
LangConfig makeTypeScript() {
    ...
    for (auto& k : std::vector<std::string>{"any","as","asserts","bigint","declare",
            "infer","is","keyof","never","namespace","object","override","readonly",
            "require","satisfies","symbol","type","unique","unknown","using"})
...
LangConfig makeGo() {
    return {
        {"break","case","chan","const","continue","default","defer","else",
         ...
         "var","any","bool","byte","comparable","complex64","complex128",
        ...
        "//", "/*", "*/",
        /*tripleString=*/false, /*charLiterals=*/true, /*hasPreprocessor=*/false,
        /*digitSep=*/'_',
    };
}
...
LangConfig makeRust() {
    return {
        {"as","async","await","break","const","continue","crate","dyn","else",
         ...
         "println","eprintln","panic","assert","assert_eq","assert_ne",
...
LangConfig makeAda() {
    return {
        {"abort","abs","abstract","accept","access","aliased","all","and",
         ...
         "Boolean","Integer","Float","Character","String","Natural","Positive",
         "Duration","Wide_Character","Wide_String","True","False"},
        "--", "", "",
        /*tripleString=*/false, /*charLiterals=*/true, /*hasPreprocessor=*/false,
        /*digitSep=*/'_',
    };
...
struct Tokenizer {
    ...
    bool isKw(const std::string& word) const {
        ...
        if (cfg.lineComment == "--") {
            std::string lo = word;
            for (char& ch : lo) ch = static_cast<char>(std::tolower(static_cast<unsigned char>(ch)));
    ...
    std::string run(const std::string& src) {
        ...
        for (size_t i = 0; i < n; ) {
            ...
            case State::Number: {
                ...
                bool cont = std::isdigit(static_cast<unsigned char>(c))
                         || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')
                         || (c == 'x' || c == 'X' || c == 'o' || c == 'O'
                             || c == 'b' || c == 'B')
                         || c == '.'
                         || (cfg.digitSep != '\0' && c == cfg.digitSep)
                         || c == 'e' || c == 'E' || c == 'p' || c == 'P'
                         || ((c == '+' || c == '-') && i > 0
                             && (src[i-1] == 'e' || src[i-1] == 'E'
                              || src[i-1] == 'p' || src[i-1] == 'P'))
            ...
            case State::BlockComment: {
                ...
                if (!cfg.blockClose.empty()
                    && src.compare(i - cfg.blockClose.size(), cfg.blockClose.size(), cfg.blockClose) == 0) {
                    reset();
                    state = State::Normal;
                }
                break;
...
std::vector<std::string> highlightCode(const std::vector<std::string>& lines,
    ...
    for (char c : highlighted) {
        if (c == '\n') { result.push_back(std::move(cur)); cur.clear(); }
        else cur += c;
    }
